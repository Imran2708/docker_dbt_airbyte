...
with __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB1 as (

-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: "DOC_DBT_AIR".IMRAN._AIRBYTE_RAW_SRC_COVID_EPIDEMIOLOGY
select
...
from "DOC_DBT_AIR".IMRAN._AIRBYTE_RAW_SRC_COVID_EPIDEMIOLOGY as table_alias
-- SRC_COVID_EPIDEMIOLOGY
where 1 = 1

),  __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB1
select
    cast(KEY as 
    varchar
) as KEY,
    ...
    _AIRBYTE_EMITTED_AT,
    convert_timezone('UTC', current_timestamp()) as _AIRBYTE_NORMALIZED_AT
from __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB1
-- SRC_COVID_EPIDEMIOLOGY
where 1 = 1

),  __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB2
select
    ...
    varchar
)) as _AIRBYTE_SRC_COVID_EPIDEMIOLOGY_HASHID,
    tmp.*
from __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB2 tmp
-- SRC_COVID_EPIDEMIOLOGY
where 1 = 1

)-- Final base SQL model
-- depends_on: __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB3
select
    KEY,
    DATE,
    NEW_TESTED,
    ...
    _AIRBYTE_EMITTED_AT,
    convert_timezone('UTC', current_timestamp()) as _AIRBYTE_NORMALIZED_AT,
    _AIRBYTE_SRC_COVID_EPIDEMIOLOGY_HASHID
from __dbt__cte__SRC_COVID_EPIDEMIOLOGY_AB3
-- SRC_COVID_EPIDEMIOLOGY from "DOC_DBT_AIR".IMRAN._AIRBYTE_RAW_SRC_COVID_EPIDEMIOLOGY
where 1 = 1

            ) order by (_AIRBYTE_EMITTED_AT)
      );
    alter table "DOC_DBT_AIR".IMRAN."SRC_COVID_EPIDEMIOLOGY" cluster by (_AIRBYTE_EMITTED_AT);